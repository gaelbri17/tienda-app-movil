...
class Validator
{
    ...
    */
    public static function validatePassword($value)
    {
        ...
        if (strlen($value) < 8) {
            self::$password_error = 'The password has less than 8 characters';
            return false;
        } elseif (strlen($value) <= 72) {
            return true;
        } else {
            self::$password_error = 'The password has more than 72 characters';
            return false;
        }
    }
    ...
    */
    ...
    {
        if (trim($value) == '') {
            self::$search_error = 'Type a search value';
            return false;
        } elseif(str_word_count($value) > 3) {
            self::$search_error = 'The seach value has more than 3 words';
            return false;
        } elseif (self::validateString($value)) {
            self::$search_value = $value;
            return true;
        } else {
            self::$search_error = 'The search value has prohibited characters';
            return false;
        }
    }
