...
const ID_MARCA = document.getElementById('idMarca');
const NOMBRE_MARCA = document.getElementById('nombreMarca');
const IMG_MARCA = document.getElementById('imgMarca');
...
const fillTable = async (form = null) => {
    ...
    if (DATA.status) {
        ...
        DATA.dataset.forEach(row => {
            ...
            TABLE_BODY.innerHTML += `
                <tr>
                    <td><img src="${SERVER_URL}images/categorias/${row.marca_img}" height="50"></td>
                    <td>${row.marca}</td>
                    <td>
...
const openUpdate = async (id) => {
    ...
    if (DATA.status) {
        ...
        IMG_MARCA.values = ROW.marca_img;
...
const openDelete = async (id) => {
    ...
    const RESPONSE = await confirmAction('¿You want to remove the brand permanently?');
