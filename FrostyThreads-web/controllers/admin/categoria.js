...
const fillTable = async (form = null) => {
    ...
    if (DATA.status) {
        ...
        DATA.dataset.forEach(row => {
            ...
            TABLE_BODY.innerHTML += `
                <tr>
                    <td><img src="${SERVER_URL}images/categorias/${row.categoria_img}" height="50"></td>
                    <td>${row.categoria}</td>
...
const openUpdate = async (id) => {
    ...
    if (DATA.status) {
        ...
        NOMBRE_CATEGORIA.value = ROW.categoria;
        DESCRIPCION_CATEGORIA.value = ROW.categoria_descripcion;
    } else {
        sweetAlert(2, DATA.error, false);
    }
}
...
const openDelete = async (id) => {
    ...
    const RESPONSE = await confirmAction('¿You want to remove the category permanently?');
